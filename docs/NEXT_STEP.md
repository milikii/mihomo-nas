...
- 若继续施工，优先选择：
  ...
  - 继续补 `internal/app` / `core-upgrade-alpha` 的剩余低覆盖热点，优先 `ApplyRules`、`InstallSelf`、`RuntimeAudit`、`fetchMihomoReleases`、`selectLinuxReleaseAsset`、`downloadReleaseAsset` 与 `naturalCompare` 的尾部分支；`controllerRequest`、`hasReadyProviders`、`networkMenu` 共享输入链路已完成当前轮次收口。
