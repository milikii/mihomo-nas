...
- 单元与 focused 测试已经覆盖核心配置、状态、provider、rules-repo、runtime 渲染、app 命令编排、CLI 分发与多组失败路径；`internal/app` 经过最近十个小闭环已提升到 91.1% 语句覆盖率。
...
- 若继续施工，优先选择：
  ...
  - 继续补 `internal/app` 的剩余低覆盖热点，优先 `apply-rules`、`networkMenu`、`testNodeDelay` 的保守边界分支。
