...
- 本轮已补上 `ApplyRules` 的 chain flush、已有 jump、DNS hijack、clear-rules 删除失败传播，以及 config/provider/runtime 的小边界测试；全量 `go test ./...`、`go test -cover ./...` 和 `go build` 已通过。
...
- 质量硬化已经把 `apply-rules` 的关键失败传播、route 编排幂等边界、rules-repo / provider 边界、config/state 缺省边界补到可测；当前没有新的代码缺口，下一步只做收口、push 和后续人工 cutover 等待。
