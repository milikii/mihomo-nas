...
- 下一闭环进入阶段 5：代码结构收口。
- 当前已完成第六十一刀：`persist_project_sync_settings` 的 MANAGER_SYNC 三连写已抽到共用 helper。
- 目标不是扩功能，而是在不改变行为的前提下，继续消除 `persist_project_sync_settings` / `reset_project_sync_settings` 周边的结构重复逻辑。

## 下一最小闭环

- 让 `reset_project_sync_settings` 复用 `write_manager_sync_settings`
- 保持现有输出文本、顺序与退化行为不变
...
- `reset_project_sync_settings` 不再内联 MANAGER_SYNC 三连重置
