...
- 首版范围限定为总览、节点、配置、规则、服务、日志和 `core-upgrade-alpha`；节点测速复用 controller delay API，raw 配置编辑直接操作 `/etc/minimalist/config.yaml` 并先做 YAML 解析校验；不做多用户、权限分级、TLS 终止或独立前端构建链
