...
- 2026-05-16 WebUI 已补齐节点测速与 raw config 编辑：节点页显示全部启用节点测速和单节点测速结果；配置页可直接编辑 `/etc/minimalist/config.yaml`，保存前做 YAML 解析校验；live `/api/nodes/test` 和 `/api/config/raw` smoke 已通过。
