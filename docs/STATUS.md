...
- 本轮验证结果：focused `go test ./internal/app -run 'Test(RemoveNodeRejectsReferencedManualNode|RemoveNodeRejectsSubscriptionNode|ListNodesAndRemoveNodePersistUpdatedState|RenameNodeRejectsSubscriptionNode|SetNodeEnabledUpdatesManualNodesAndRejectsSubscriptionNodes)'` 与全量 `go test ./...` 通过。
