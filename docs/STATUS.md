...
- 当前主线已进入阶段 5，已完成第五十六刀：`cleanup_project_install_tree_metadata` 的备份垃圾清理已抽到共用 helper。
...
- `cleanup_project_sync_runtime` 的 unit 文件删除仍与 timer 停用、systemd reload 内联耦合，阶段 5 后续可继续收口
