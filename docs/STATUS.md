...
- 当前主线已进入阶段 5，已完成第五十七刀：`cleanup_project_sync_runtime` 的 unit 文件删除已抽到共用 helper。
...
- `activate_project_sync_runtime` / `cleanup_project_sync_runtime` 的 systemd reload 仍重复内联，阶段 5 后续可继续收口
